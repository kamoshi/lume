...
/// Entries here take precedence over the built-in defaults; anything not present
/// falls back to the hardcoded values in [`Parser::infix_bp`].
///
/// Build one with [`Parser::new`] and optionally populate `bp_table` before
/// calling [`Parser::parse_program`] / [`Parser::parse_expr`].
pub struct Parser {
    pub bp_table: HashMap<String, (u8, u8)>,
}
...
impl Parser {
    pub fn parse_program(&self, tokens: &[Spanned]) -> Result<Program, ParseError> {
        let mut ptr = 0;
        ...
        while matches!(first_token(&tokens[ptr..]), Some(Token::Use)) {
            ...
            let (n, u) = Self::parse_use(&tokens[ptr..])?;
            ptr += n;
            uses.push(u);
        }
        ...
        loop {
            ...
            match first_token(&tokens[ptr..]) {
                Some(Token::Type) => {
                    let (n, mut td) = Self::parse_typedef(&tokens[ptr..])?;
                    ptr += n;
                    td.doc = pending_doc;
                    items.push(TopItem::TypeDef(td));
                }
                Some(Token::Let) => {
                    let (n, mut b) = self.parse_binding(&tokens[ptr..])?;
                    ptr += n;
                    b.doc = pending_doc;
                    ...
                    if matches!(first_token(&tokens[ptr..]), Some(Token::And)) {
                        ...
                        while matches!(first_token(&tokens[ptr..]), Some(Token::And)) {
                            ...
                            ptr += and_doc_n;
                            let (n, mut next) = self.parse_binding(&tokens[ptr..])?;
                            ptr += n;
                            next.doc = and_doc;
                            group.push(next);
                        }
                ...
                Some(Token::Trait) => {
                    let (n, mut td) = Self::parse_trait_def(&tokens[ptr..])?;
                    ptr += n;
                    td.doc = pending_doc;
                ...
                Some(Token::Use) => {
                    ...
                    let (n, mut id) = self.parse_impl_def(&tokens[ptr..])?;
                    ptr += n;
                    id.doc = pending_doc;
        ...
        let exports = if matches!(first_token(&tokens[ptr..]), Some(Token::Pub)) {
            ptr += 1;
            let (n, exports) = self.parse_expr(&tokens[ptr..])?;
            ptr += n;
            exports
        } else {
            Expr {
    ...
    fn parse_impl_def(&self, tokens: &[Spanned]) -> Result<(usize, ImplDef), ParseError> {
        let mut ptr = 0;
        ptr += consume(&tokens[ptr..], &Token::Use)?;
        ...
        let (impl_constraints, type_name, target_type, type_name_span) = {
            let saved = ptr;
            match Self::try_parse_impl_constraints(&tokens[ptr..]) {
                Some((n, constraints)) => {
                    ptr += n;
                    let type_name_span = span(&tokens[ptr..]);
                    let (n, type_name, target_type) = Self::parse_impl_target_type(&tokens[ptr..])?;
                    ptr += n;
        ...
        while !matches!(first_token(&tokens[ptr..]), Some(Token::RBrace) | None) {
            ...
            ptr += consume(&tokens[ptr..], &Token::Equal)?;
            let (n, value) = self.parse_expr(&tokens[ptr..])?;
            ptr += n;
    ...
    fn parse_impl_target_type(tokens: &[Spanned]) -> Result<(usize, String, Type), ParseError> {
        ...
        if matches!(first_token(tokens), Some(Token::LBrace)) {
            ...
            let type_name = Self::type_to_canonical_string(&ty);
        ...
        let type_name = Self::type_to_canonical_string(&target_type);
    ...
    fn type_to_canonical_string(ty: &Type) -> String {
        match ty {
            ...
            Type::App { callee, arg } => {
                let callee_s = Self::type_to_canonical_string(callee);
                let arg_s = Self::type_to_canonical_string(arg);
            ...
            Type::Func { param, ret } => {
                format!(
                    "{} -> {}",
                    Self::type_to_canonical_string(param),
                    Self::type_to_canonical_string(ret)
                )
            }
            Type::Record(rt) => {
                ...
                let fields: Vec<String> = sorted_fields
                    .iter()
                    .map(|f| format!("{}: {}", f.name, Self::type_to_canonical_string(&f.ty)))
                    .collect();
                if rt.open {
                    format!("{{ {}, .. }}", fields.join(", "))
                } else {
    ...
    fn parse_typedef(tokens: &[Spanned]) -> Result<(usize, TypeDef), ParseError> {
        ...
        if matches!(first_token(&tokens[ptr..]), Some(Token::Bar)) {
            ...
            while matches!(first_token(&tokens[ptr..]), Some(Token::Bar)) {
                ptr += 1; // consume `|`
                let (n, v) = Self::parse_variant(&tokens[ptr..])?;
                ptr += n;
                variants.push(v);
            }
    ...
    fn parse_binding(&self, tokens: &[Spanned]) -> Result<(usize, Binding), ParseError> {
        let mut ptr = 0;
        ptr += consume(&tokens[ptr..], &Token::Let)?;
        ...
        while !matches!(
            ...
            let (n, param, param_ty) = Self::parse_binding_param(&tokens[ptr..])?;
            ptr += n;
        ...
        let has_head_annotations =
            sugar_ret_ty.is_some() || params.iter().any(|(_, ty)| ty.is_some());
        ...
        let (n, mut value) = self.parse_expr(&tokens[ptr..])?;
        ptr += n;

        if !params.is_empty() {
            value = Self::desugar_binding_params(value, &params);
        }

        let ty = explicit_ty.or_else(|| Self::synthesise_sugar_binding_type(&params, sugar_ret_ty));
    ...
    fn parse_binding_param(
        tokens: &[Spanned],
    ) -> Result<(usize, Pattern, Option<Type>), ParseError> {
        if matches!(first_token(tokens), Some(Token::LParen)) {
            if tokens
                .get(1)
    ...
    fn parse_expr(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        // `let pattern = value in body`
        if matches!(first_token(tokens), Some(Token::Let)) {
            if let Ok((n, expr)) = self.try_parse_let_in(tokens) {
                return Ok((n, expr));
            }
        }
        ...
        if let Ok((n, expr)) = self.try_parse_lambda(tokens) {
            return Ok((n, expr));
        }
    ...
    fn try_parse_let_in(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        ...
        let (n, body) = self.parse_expr(&tokens[ptr..])?;
        ptr += n;
    ...
    fn try_parse_lambda(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
    ...
    fn infix_bp(&self, tok: &Token) -> Option<(u8, u8)> {
        // Check the user-supplied table first so that any operator (including
        // custom ones defined via `Token::Operator`) can have its precedence
        // overridden at runtime.
        if let Some(op_str) = token_to_op_name(tok) {
            if let Some(&bp) = self.bp_table.get(&op_str) {
                return Some(bp);
            }
        }
    ...
    fn parse_pratt(&self, tokens: &[Spanned], min_bp: u8) -> Result<(usize, Expr), ParseError> {
        let mut ptr = 0;
        ...
        if matches!(first_token(&tokens[ptr..]), Some(Token::Not)) {
            ...
            ptr += 1;
            let (n, operand) = self.parse_pratt(&tokens[ptr..], 80)?;
            ptr += n;
            return Ok((
                ptr,
                Expr {
        ...
        let (n, mut lhs) = self.parse_apply(&tokens[ptr..])?;
        ptr += n;

        // ── Infix loop ────────────────────────────────────────────────────────────
        while let Some(tok) = first_token(&tokens[ptr..]) {
            let (l_bp, r_bp) = match self.infix_bp(tok) {
                Some(bp) => bp,
                None => break,
            };
            ...
            let op = Self::token_to_binop(&tokens[ptr].token).unwrap();
            ptr += 1; // consume the operator

            let (n, rhs) = self.parse_pratt(&tokens[ptr..], r_bp)?;
            ptr += n;
    ...
    fn parse_apply(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        let mut ptr = 0;
        let (n, mut expr) = self.parse_atom(&tokens[ptr..])?;
        ptr += n;

        // Greedily consume additional atoms as arguments
        loop {
            if !Self::can_start_atom(&tokens[ptr..]) {
                break;
            }
            ...
            match self.parse_atom(&tokens[ptr..]) {
                Ok((n, arg)) => {
    ...
    /// ```
    fn parse_atom(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        match first_token(tokens) {
            ...
            Some(Token::TypeIdent(name)) => {
                ...
                let payload = if matches!(first_token(&tokens[ptr..]), Some(Token::LBrace)) {
                    let (n, rec) = self.parse_record_expr(&tokens[ptr..])?;
                    ptr += n;
                    Some(Box::new(rec))
                } else {
                    None
            ...
            Some(Token::LBrace) => {
                let (n, expr) = self.parse_record_expr(tokens)?;
                Ok((n, expr))
            }

            // ── List ──────────────────────────────────────────────────────────────
            Some(Token::LBracket) => {
                let (n, expr) = self.parse_list_expr(tokens)?;
                Ok((n, expr))
            }
            ...
            Some(Token::LParen) => {
                ...
                let (n, inner) = self.parse_expr(&tokens[ptr..])?;
                ptr += n;
                ptr += consume(&tokens[ptr..], &Token::RParen)?;
                Ok((ptr, inner))
            }
            ...
            // ── If expression ─────────────────────────────────────────────────────
            Some(Token::If) => {
                let (n, expr) = self.parse_if(tokens)?;
                Ok((n, expr))
            }

            // ── Match expression (series of `| pat -> body` arms) ────────────────
            Some(Token::Bar) => {
                let (n, expr) = self.parse_match(tokens)?;
                Ok((n, expr))
            }

            // ── Match-in expression: `match expr in | pat -> body ...` ───────────
            Some(Token::Match) => {
                let (n, expr) = self.parse_match_expr(tokens)?;
                Ok((n, expr))
            }
    ...
    fn parse_record_expr(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        ...
        while !matches!(first_token(&tokens[ptr..]), Some(Token::RBrace) | None) {
            ...
            if matches!(first_token(&tokens[ptr..]), Some(Token::DotDot)) {
                ...
                let (n, expr) = self.parse_expr(&tokens[ptr..])?;
                ptr += n;
            ...
            let (n, field) = self.parse_record_field(&tokens[ptr..])?;
            ptr += n;
    ...
    /// `[1, 2, 3]`  or  `[]`
    fn parse_list_expr(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        ...
        while !matches!(first_token(&tokens[ptr..]), Some(Token::RBracket) | None) {
            if matches!(first_token(&tokens[ptr..]), Some(Token::DotDot)) {
                ptr += 1; // consume ..
                let (n, spread) = self.parse_expr(&tokens[ptr..])?;
                ptr += n;
                entries.push(ListEntry::Spread(spread));
            } else {
                let (n, item) = self.parse_expr(&tokens[ptr..])?;
                ptr += n;
    ...
    fn parse_if(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        ...
        let (n, cond) = self.parse_expr(&tokens[ptr..])?;
        ptr += n;

        ptr += consume(&tokens[ptr..], &Token::Then)?;

        let (n, then_branch) = self.parse_expr(&tokens[ptr..])?;
        ptr += n;

        ptr += consume(&tokens[ptr..], &Token::Else)?;

        let (n, else_branch) = self.parse_expr(&tokens[ptr..])?;
        ptr += n;
    ...
    /// `(| pattern guard? -> body)+`
    fn parse_match(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        ...
        while matches!(first_token(&tokens[ptr..]), Some(Token::Bar)) {
            ...
            let guard = if matches!(first_token(&tokens[ptr..]), Some(Token::If)) {
                ptr += 1; // consume `if`
                let (n, g) = self.parse_expr(&tokens[ptr..])?;
                ptr += n;
                Some(g)
            } else {
                None
    ...
    fn parse_match_expr(&self, tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
        ...
        let (n, scrutinee) = self.parse_pratt(&tokens[ptr..], 0)?;
        ptr += n;
...
//
// These thin wrappers preserve the original call signatures used throughout the
// codebase.  Callers that need custom operator precedences should construct a
// [`Parser`] directly and call methods on it.

/// Parse a complete Lume source file.
pub fn parse_program(tokens: &[Spanned]) -> Result<Program, ParseError> {
    Parser::new().parse_program(tokens)
}

/// Parse a single expression.
pub fn parse_expr(tokens: &[Spanned]) -> Result<(usize, Expr), ParseError> {
    Parser::new().parse_expr(tokens)
}

/// Parse a single `let` binding.
pub fn parse_binding(tokens: &[Spanned]) -> Result<(usize, Binding), ParseError> {
    Parser::new().parse_binding(tokens)
}
